    ...
    if (ajax.readyState === 4) {
      if (ajax.status == "200") {
        ...
        for (var scWorkflow of scWorkflows) {
          var scWorkflowTitle = scWorkflow.querySelector(".scPaneHeader")
            .innerText;
          wfChecksum +=
            "-workflow:" + scWorkflowTitle.replace(" ", "").toLowerCase();

          var wfStates = scWorkflow.querySelectorAll(".scBackground");

          for (var wfState of wfStates) {
            var wfStateTitle = wfState.querySelector(".scSectionCenter")
              .innerText;
            var wfStateTitleCount = wfStateTitle.split(" - (")[1].toLowerCase();
            wfStateTitleCount = wfStateTitleCount
              .replace(")", "")
              .replace(" item", "")
              .replace("s", "");
            ...
            wfChecksum +=
              "-state:" +
              wfStateTitle.split(" - ")[0].replace(" ", "").toLowerCase() +
              ":" +
              wfNotification;
          }
        }
      ...
      if (global.isLaunchpad) {
        ...
        global.workboxLaunchpad
          ? global.workboxLaunchpad.insertAdjacentHTML("afterbegin", html)
          : false;
      } else {
        ...
        if (scNotificationBell && wfNotification > 0) {
          ...
          scNotificationBell.setAttribute(
            "title",
            "You have " +
              wfNotification +
              " notification" +
              setPlural(wfNotification) +
              " in your workbox"
          );
